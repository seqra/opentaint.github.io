...
1. **Following data across file and class boundaries** — through DTO field access and method chains.
...
The first place conventional tools lose the trail is at class boundaries. When a controller delegates to a service, the data crosses files, passes through DTO field access, and chains through multiple methods before reaching a sink. An analyzer that only tracks flow within a single method or file sees a dead end.

Consider a campaign management endpoint that lets users preview custom templates. The controller receives a JSON request body and delegates to a service:
