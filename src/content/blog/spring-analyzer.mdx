...
`template.process()` is a conditionally dangerous method: it is a sink only when the receiver permits class loading. That permission is set inside the Freemarker engine's `@Autowired` constructor — not at the call site, not in the rule. An analyzer that cannot resolve which bean is wired in and walk its constructor either flags every call (noise) or none (missed RCE).
