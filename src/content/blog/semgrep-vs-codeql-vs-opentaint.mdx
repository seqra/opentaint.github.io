...
- **Whole-program analysis requires a build.** OpenTaint analyzes compiled programs — that is the source of its depth. Pattern-only scans skip the build step and run faster, but cannot follow data across the boundaries tested here.
...
Each tool plateaus at a different level of analytical depth. Semgrep CE handles syntax matching and local taint tracking — levels 1–2 — but stops at function boundaries. Semgrep Code extends through inter-procedural analysis and field sensitivity — levels 3–4 — but does not follow builder patterns or virtual dispatch. CodeQL covers most cases through level 4, but its analysis depth limits surface at deep field chains and virtual calls. OpenTaint tracks data through all five levels — including builder state, constructor chains, and interface dispatch — using the same pattern rules throughout.

The key design difference: in Semgrep, the rule author declares the dataflow model — sources, sinks, sanitizers. In OpenTaint, the engine infers it. In these five cases, a pattern that mentions a parameter and a return statement is enough for the engine to recover the full flow — across assignments, method calls, and object boundaries. The simpler the rule, the easier it is for an AI agent to write and maintain — OpenTaint's pattern rules are sufficient for all five levels, which means an agent that can describe the source and sink can cover what other tools need hand-crafted taint configurations for.

These five levels measure a general problem: ordinary vulnerabilities hidden behind ordinary indirection. The deeper the analysis, the more boundaries it can see through — and the fewer findings it drops or fabricates. OpenTaint's result here depends on whole-program dataflow depth. That buys simpler rules, but it also means the engine must be precise at call boundaries, field loads, and virtual dispatch. Explicit taint rules offer more local control, at the cost of more rule logic.
