...
The same endpoint, but with the input assigned to a local variable before the return. That single step breaks AST-pattern analyzers: the tool now needs local dataflow tracking.
...
- ❌ **Semgrep (pattern)**: Misses the vulnerability — AST-pattern analyzers cannot follow the assignment.
