...
Before tackling cross-endpoint flows, we start with what happens within a single HTTP request — following data through DI boundaries and recognizing when a method call is actually dangerous based on the receiver state the rule's condition names.
...
Resolving the chain across DI boundaries is necessary but not sufficient. The next question is whether the call at the end of the chain is genuinely dangerous — and for Spring, that often hinges on what the bean's `@Autowired` constructor did to the receiver.

### When Autowired Constructors Matter

`template.process()` is a conditionally dangerous method: it is exploitable only when the receiver permits class loading. That permission is set inside the Freemarker engine's `@Autowired` constructor — not at the call site, not in the rule. An analyzer that cannot resolve which bean is wired in and walk its constructor either flags every call (noise) or none (missed RCE).
