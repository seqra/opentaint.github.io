...
Each level requires a deeper understanding of Spring's framework semantics. The engine models all three.

## Single-Request Flows

The first two challenges — dependency injection and configuration-aware sinks — occur within a single HTTP request. The data enters through a controller, crosses class and service boundaries, and reaches a sink before the response returns.

For JVM languages, OpenTaint operates on bytecode rather than source text. This requires a successful build before scanning, but gives precise resolution of inheritance, generics, and library calls — precision that matters in Spring, where runtime behavior depends on bean wiring, annotation metadata, and framework conventions that AST-only tools treat as opaque.
...
We deliberately over-approximate in several areas to preserve coverage. Deliberate false positives can appear from:
