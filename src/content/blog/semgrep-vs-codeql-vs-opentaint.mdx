---
title: "Semgrep vs. CodeQL vs. OpenTaint: Five Levels of XSS Detection Depth"
...
Three tools, one test application — an intentionally vulnerable Spring Boot project designed to isolate different aspects of XSS detection. Each example measures two things:
...
**Semgrep** matches patterns syntactically, with taint-analysis support and broader inter-procedural coverage in its commercial edition. Results below distinguish Semgrep CE and Semgrep Code where they diverge. **CodeQL** runs semantic analysis through a dedicated query language; we use its default `java/xss` rule. **OpenTaint** interprets Semgrep-style patterns as dataflow queries — metavariables are tracked as program values through assignments, method calls, field chains, and virtual dispatch.
...
The key design difference: in Semgrep, the rule author declares the dataflow model — sources, sinks, sanitizers. In OpenTaint, the engine infers it. In these five cases, a pattern that mentions a parameter and a return statement is enough for the engine to recover the full flow — across assignments, method calls, and object boundaries.

OpenTaint's result here depends on whole-program dataflow depth. That buys simpler rules, but it also means the engine must be precise at call boundaries, field loads, and virtual dispatch. Explicit taint rules offer more local control, at the cost of more rule logic.

## Limitations

This comparison has deliberate constraints worth naming.

- **Five cases, one application, one vulnerability class.** XSS in a Spring Boot project isolates analytical depth but says nothing about language breadth, rule coverage, or performance at scale. A tool that handles all five levels here may still miss patterns in other frameworks or languages.
...
- **Whole-program analysis has cost.** OpenTaint's depth depends on building and analyzing the full program. That requires a successful compile and takes longer than pattern-only scans. For large monorepos, this is a real tradeoff against scan time.

These five levels measure a general problem: ordinary vulnerabilities hidden behind ordinary indirection. The deeper the analysis, the more boundaries it can see through — and the fewer findings it drops or fabricates.
